...
//       if (filterDate) {
//         if (!item.priority1Date) return false;

//         // 1. สร้าง Object Date จากข้อมูลทั้งสองฝั่ง
//         const itemDate = new Date(item.priority1Date);
//         const searchDate = new Date(filterDate);

//         // 2. เทียบ วัน/เดือน/ปี ตามเวลาเครื่อง (Local Time) 
//         // วิธีนี้จะแก้ปัญหาเรื่องโซนเวลาเพี้ยนได้ 100%
//         const isSameDay = 
...
//         note: updateForm.note // รายละเอียดเพิ่มเติม
//       });
//       setSelectedTask(null);
//     }
//     else if (selectedItemIds.length > 0) {
...
//     setShowModal(true);
//   };
//   const handleRedownload = (targetBatchId, batchDate) => {
//     // 1. ดึงข้อมูล Appointment ทั้งหมดที่อยู่ใน Batch นี้ออกมา
//     // สมมติว่าใน appointment มี field 'batchId' เก็บอยู่
...
//     // 2. เรียงลำดับข้อมูลให้เหมือนตอน Export ครั้งแรก (สำคัญมาก เพื่อให้ไฟล์เหมือนเดิม)
//     // เรียงตาม วันนัดหลัก -> วันที่สร้าง
...
//     // 3. Map ข้อมูลเข้า Format Excel (ใช้ Code ชุดเดียวกับตอน Export ครั้งแรก)
...
//       "เหตุผลที่ไม่ได้จอง": appt.rejectReason || "", // เผื่อกรณีมีการกรอกข้อมูลเพิ่มแล้ว
...
//     // 5. ตั้งชื่อไฟล์ (ใช้ Batch ID หรือวันที่เดิมเพื่อให้ track ง่าย)
//     // เช่น BatchExport-2023-11-25.xlsx
...
//                       onClick={() => handleRedownload(currentBatchInfo.id, currentBatchInfo.date)}
//                     >
...
//                           <td>{(item.status === 'SENT' || item.status === 'REJECTED') ?
//                             (<input type="checkbox" checked={selectedItemIds.includes(item.id)} onChange={() => handleSelectItem(item.id)} />)
//                             : <CheckCircle2 size={16} className="text-muted" />}</td>
...
//                           <td><StatusBadge status={item.status} /></td>
//                           <td className="text-end">{(item.status === 'SENT' || item.status === 'REJECTED') &&
//                             (<button className="btn btn-outline-primary btn-sm rounded-pill px-2 py-1" style={{ fontSize: '0.8rem' }} onClick={() => handleOpenUpdate(item)}>อัปเดต</button>)}</td>
//                         </tr>
//                       ))}
//                     </tbody>
//                   </table>
...
//                     <textarea className="form-control" rows="2" placeholder="เช่น แนะนำให้จองใหม่เดือนหน้า..." value={updateForm.note}
//                       onChange={(e) => setUpdateForm({ ...updateForm, note: e.target.value })}></textarea>
//                   </div>
//                 </div>
//               )}
//             </div>
...
import {
  ...
  Check, X, Save, ChevronLeft, ChevronRight
} from "lucide-react";
import * as XLSX from "xlsx";
...
// ฟังก์ชันแปลงวันที่เป็น YYYY-MM-DD แบบ Local Time (แก้ปัญหา Timezone)
...
const getSLAStatus = (createdDate, status) => {
  ...
  if (diffHours > 48) {
    return (
      <span className="badge bg-danger-subtle text-danger border border-danger-subtle ms-2 d-inline-flex align-items-center gap-1">
        <AlertTriangle size={10} /> ล่าช้า ({Math.floor(diffHours / 24)} วัน)
      </span>
    );
  } else if (diffHours > 24) {
...
export default function AdminTracking() {
  ...
  // --- States: Filters ---
  const [searchTerm, setSearchTerm] = useState("");
  const [filterSentDate, setFilterSentDate] = useState("");     // วันที่ส่ง
  const [filterBookingDate, setFilterBookingDate] = useState(""); // วันที่นัด
  const [filterHospital, setFilterHospital] = useState("ALL");
  const [filterUrgentOnly, setFilterUrgentOnly] = useState(false);

  // --- States: Data Selection & Update ---
  const [selectedTask, setSelectedTask] = useState(null);
  const [selectedBatchId, setSelectedBatchId] = useState(null);
  const [selectedItemIds, setSelectedItemIds] = useState([]);
  ...
  // 🔥 Logic การกรองข้อมูล (Refactored)
  const filteredList = useMemo(() => {
    if (activeTab === 'BATCH') return [];
  ...
  const handleSaveUpdate = () => {
    ...
    if (selectedTask) {
      ...
      selectedItemIds.forEach(id => {
        const originalItem = appointments.find(a => a.id === id);
        // กรณี Confirmed แบบกลุ่ม ให้ใช้วันที่ P1 ของคนนั้นๆ ถ้าไม่ได้ระบุวันใหม่
        const targetDate = updateForm.status === 'CONFIRMED' 
          ? (originalItem?.priority1Date || updateForm.confirmedDate) 
          : null;
            ...
            <div className="d-flex gap-3 align-items-center">
              ...
              <div className="position-relative" style={{ width: '160px' }}>
                <input 
                  ...
                  title="กรองตามวันที่ส่งเรื่อง (Sent Date)"
                  value={filterSentDate} 
                  onChange={e => setFilterSentDate(e.target.value)} 
                />
                <span className="position-absolute top-0 start-0 translate-middle-y badge bg-light text-muted border px-1 ms-2 mt-1" style={{fontSize: '0.6rem'}}>
                  วันที่ส่ง
                </span>
                {filterSentDate && (
                  <button className="btn-close position-absolute top-50 end-0 translate-middle-y me-2 small" style={{ fontSize: '0.7rem' }} onClick={() => setFilterSentDate("")} />
                )}
              </div>
              ...
              <div className="position-relative" style={{ width: '160px' }}>
                <input 
                  ...
                  className="form-control text-muted small" 
                  style={{fontSize: '0.9rem'}}
                  title="กรองตามวันที่ขอนัด (Booking Date)"
                ...
                {filterBookingDate && (
                  <button className="btn-close position-absolute top-50 end-0 translate-middle-y me-2 small" style={{ fontSize: '0.7rem' }} onClick={() => setFilterBookingDate("")} />
                )}
              </div>
                      ...
                      <td className="text-end">
                        {activeTab === 'ACTION' && (
                          <button className="btn btn-primary btn-sm rounded-pill px-3" onClick={() => handleOpenUpdate(item)}>
                            {item.status === 'REJECTED' ? 'แก้ไขงาน' : '📝 อัปเดตผล'}
                          </button>
                        )}
                        {activeTab === 'HISTORY' && (
            ...
            <div className="modal-body">
              ...
              {updateForm.status === 'CONFIRMED' ? (
                <div className="animate-slide-up">
                  ...
                  {selectedTask && (
                    <div className="d-flex gap-3 mb-4">
                      ...
                      ) : (<div className="date-select-card flex-grow-1 disabled bg-light text-muted border-dashed d-flex align-items-center justify-content-center"><small>ไม่มีวันนัดสำรอง</small></div>)}
                    </div>
                  )}
                  <div className="bg-light p-3 rounded border">
                    <div className="row g-3">
                      <div className="col-6"><label className="small text-muted mb-1">วันที่ (กำหนดเอง)</label><input type="date" className="form-control bg-white" value={updateForm.confirmedDate} onChange={e => setUpdateForm({ ...updateForm, confirmedDate: e.target.value })} /></div>
                      <div className="col-6"><label className="small text-muted mb-1">เวลาที่นัด</label><input type="time" className="form-control bg-white" value={updateForm.confirmedTime} onChange={e => setUpdateForm({ ...updateForm, confirmedTime: e.target.value })} /></div>
                ...
                <div className="bg-danger-soft p-3 rounded border border-danger-subtle animate-slide-up">
                  ...
                  <div className="mb-3">
                    <label className="small text-muted mb-1 fw-bold">เสนอวันนัดใหม่ (ถ้ามี)</label>
                    <input type="date" className="form-control" value={updateForm.newProposedDate} onChange={e => setUpdateForm({ ...updateForm, newProposedDate: e.target.value })} />
                  </div>
                  <div className="col-12">
                    <label className="small text-muted mb-1">รายละเอียดเพิ่มเติม</label>
                    <textarea 
